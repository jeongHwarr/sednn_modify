...
###
def create_mixture_csv(DIRECTORY, args, mode):
    ...
    if mode=='train':
        speech_dir = DIRECTORY['TR_SPEECH_DIR']
        noise_dir = DIRECTORY['TR_NOISE_DIR']
    elif mode=='test':
        speech_dir = DIRECTORY['TE_SPEECH_DIR']
        noise_dir = DIRECTORY['TE_NOISE_DIR']
...
#    magnification = args.magnification
    ...
    out_csv_path = os.path.join(workspace, "mixture_csvs", "%s.csv" % mode)
...
# =============================================================================
    ...
    f.close()
    print(out_csv_path)
    print("Create %s mixture csv finished!" % mode)
    
###
def calculate_mixture_features(DIRECTORY, args, mode):
    ...
    # Open mixture csv. 
    mixture_csv_path = os.path.join(workspace, "mixture_csvs", "%s.csv" %mode)
...
def calc_sp(audio, mode, args):
    ...
    n_win = args.window
    n_hop = n_win-n_overlap
    librosa.stft(audio)
    
    winfunc = 'hamming'
    #boxcar, triang, blackman, hamming, hann, bartlett, flattop, parzen, bohman, blackmanharris, nuttall, barthann, kaiser
    
    x = librosa.stft(audio, n_fft=n_fft, win_length = n_win, hop_length=n_hop, window=winfunc)  
    x=x.T # x_axis=frequency, y_axis=time
    if mode == 'magnitude':
...
###
def pack_features(DIRECTORY, args, mode):
    ...
    # Load all features. 
    feat_dir = os.path.join(workspace, "features", "spectrogram", mode, "%ddb" % int(snr))
    ...
    out_path = os.path.join(workspace, "packed_features", "spectrogram", mode, "%ddb" % int(snr), "data.h5")
...
def mat_2d_to_3d(x, agg_num, hop):
    ...
    (index of segment, index of frame, index of frequency bin) 
    """
...
###
def compute_scaler(DIRECTORY, args, mode):
    ...
    # Load data. 
    t1 = time.time()
    hdf5_path = os.path.join(workspace, "packed_features", "spectrogram", mode, "%ddb" % int(snr), "data.h5")
    ...
    # Write out scaler. 
    out_path = os.path.join(workspace, "packed_features", "spectrogram", mode, "%ddb" % int(snr), "scaler.p")
    create_folder(os.path.dirname(out_path))
